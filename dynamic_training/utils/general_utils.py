...
# This part is used for rescale masks
def npztoTorch(mask_path:str, resolution) -> torch.Tensor:
    mask = np.load(mask_path)['arr_0'].astype(np.int16)
    height, width = resolution[0], resolution[1]
    resized_mask =  torch.from_numpy(cv2.resize(mask, (width, height), interpolation=cv2.INTER_LINEAR))
    return resized_mask
...
def form_perplexity_tensor(perplexity_path: str) -> torch.Tensor:
